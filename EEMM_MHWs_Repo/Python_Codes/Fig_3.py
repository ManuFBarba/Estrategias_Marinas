...
cs_1= axs.contourf(LON_SA_MODEL, LAT_SA_MODEL, np.nanmean(np.where(MHW_td_ts_SA_MODEL == 0, np.NaN, MHW_td_ts_SA_MODEL), axis=2), levels, cmap=cmap, transform=proj, extend ='both')
...
outfile = r'...\Fig_3\3a_SMHW_Td.png'
...
td_diff_SA = np.nanmean(np.where(MHW_td_ts_SA_MODEL[:,:,15:30] == 0, np.NaN, MHW_td_ts_SA_MODEL[:,:,15:30]), axis=2) - np.nanmean(np.where(MHW_td_ts_SA_MODEL[:,:,0:15] == 0, np.NaN, MHW_td_ts_SA_MODEL[:,:,0:15]), axis=2)
...
outfile = r'...\Fig_3\3b_SMHW_Td_diff.png'
fig.savefig(outfile, dpi=600, bbox_inches='tight')
...
cs_1= axs.contourf(LON_SA_MODEL, LAT_SA_MODEL, MHW_max_SA_MODEL, levels, cmap=cmap, transform=proj, extend ='both')
...
outfile = r'...\Fig_3\3c_SMHW_MaxInt.png'
...
max_diff_SA = np.nanmean(MHW_max_ts_SA_MODEL[:,:,15:30], axis=2) - np.nanmean(MHW_max_ts_SA_MODEL[:,:,0:15], axis=2)
signif_max_SA = np.where(max_diff_SA >= 0.25, 1, np.NaN)
signif_max_SA = np.where(max_diff_SA <= -0.25, 1, signif_max_SA)
...
outfile = r'...\Fig_3\3d_SMHW_max_diff.png'
fig.savefig(outfile, dpi=600, bbox_inches='tight', pad_inches=0.5)
...
cs_1= axs.contourf(LON_SA_MODEL, LAT_SA_MODEL, MHW_cum_SA_MODEL, levels, cmap=cmap, transform=proj, extend ='both')
...
outfile = r'...\Fig_3\3e_SMHW_CumInt.png'
fig.savefig(outfile, dpi=600, bbox_inches='tight')
...
max_diff_SA = np.nanmean(MHW_cum_ts_SA_MODEL[:,:,15:30], axis=2) - np.nanmean(MHW_cum_ts_SA_MODEL[:,:,0:15], axis=2)
...
outfile = r'...\Fig_3\3fSMHW_Cum_diff.png'
fig.savefig(outfile, dpi=600, bbox_inches='tight')
