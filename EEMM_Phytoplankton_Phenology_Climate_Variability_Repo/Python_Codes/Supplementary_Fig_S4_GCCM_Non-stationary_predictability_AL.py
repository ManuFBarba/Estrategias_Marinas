...
###### Supplementary Fig. S4. Causal Inference (GCCM) - Non-stationary predictability 
       of phytoplankton dynamics (AL)  ########
...
## Supplementary Fig. S4a-c - CHL Wavelet analysis (SoG and Alboran Sea - AL) ##

#Preparing Dataset
# ds = xr.open_dataset(r'E:\...\CHL_L4_1Km_Data/CHL_L4_1km_AL.nc')
...
# np.save(r'E:\...\Wavelet_Data/AL.npy', data)

# Load previously-proccessed data 
ds = xr.open_dataset(r'E:\...\CHL_L4_1Km_Data/CHL_L4_1km_AL.nc')
data = np.load(r'E:\...\Wavelet_Data/AL.npy')
...
signif, fft_theor = wavelet.significance(1.5, dt, scales, 0, alpha, significance_level=0.95, wavelet=mother)
sig95 = np.ones([1, N]) * signif[:, None]
sig95 = power / sig95

glbl_power = power.mean(axis=1)
dof = N - scales  # Correction for padding at edges
glbl_signif, tmp = wavelet.significance(0.015, dt, scales, 1, alpha, significance_level=0.95, dof=dof, wavelet=mother)
...
axs[0,1].set_xlim([0, 3.75])
axs[0,1].set_xticks([0, 0.5, 1, 1.5, 2, 2.5, 3, 3.5])
...
fig.savefig(r'E:\...\Figures\Fig_S4/Fig_S4a-c.png', dpi=1200, bbox_inches='tight')



## Supplementary Fig. S4d, f - Causal Inference through GCCM (Gao et al., 2023; Sugihara et al., 2012) ##

# Loading MHW Cum Intensity (from Fernández-Barba et al. (2024)), Windiness, BMaxChl-a, and SCR datasets

MHWCumInt = np.load(r'E:\...\Annual_Data\MHWCumInt_Interp_Data/MHWCumInt_AL.npy')
MHWCumInt = np.where(np.isnan(MHWCumInt), 0, MHWCumInt)
MHWCumInt = MHWCumInt[::10, ::10, :]
Windiness = np.load(r'E:\...\Annual_Data\Windiness/Windiness_AL.npy')
Windiness = Windiness[::10, ::10, :]

BMaxChla = np.load(r'E:\...\Phenology_Metrics\AL/bloom_max_AL.npy')
# BMaxChla = BMaxChla[::10,::10, 0:25]
BMaxChla = BMaxChla[::10,::10, :]

SCR = np.load(r'E:\...\Phenology_Metrics\AL/SCR_AL.npy')
# SCR = SCR[::10,::10, 0:25]
SCR = SCR[::10,::10, :]
...
np.save(r'E:\...\GCCM_outputs\SCR_xmap_Windiness/SCR_xmap_Windiness_AL.npy', rho_X_to_Y_values)
np.save(r'E:\...\GCCM_outputs\SCR_xmap_Windiness/Windiness_xmap_SCR_AL.npy', rho_Y_to_X_values)
# np.save(r'E:\...\GCCM_outputs\libsizes/libsizes_AL.npy', lib_sizes)



# Supplementary Fig. S4d (BMaxChl-a/SCR xmap MHW Cum Intensity)
libsizes_AL = np.load(r'E:\...\GCCM_outputs\libsizes/libsizes_AL.npy')
BMaxChla_xmap_MHWCumInt_AL = np.load(r'E:\...\GCCM_outputs\BMaxChla_xmap_MHWCumInt/BMaxChla_xmap_MHWCumInt_AL.npy')
MHWCumInt_xmap_BMaxChla_AL = np.load(r'E:\...\GCCM_outputs\BMaxChla_xmap_MHWCumInt/MHWCumInt_xmap_BMaxChla_AL.npy')
SCR_xmap_MHWCumInt_AL = np.load(r'E:\...\GCCM_outputs\SCR_xmap_MHWCumInt/SCR_xmap_MHWCumInt_AL.npy')
...
for i, year in enumerate(years):
    for value in BMaxChla_xmap_MHWCumInt_AL[i, :]:
        data.append([year, value, 'BMaxChla_xmap_MHWCumInt_AL'])
    for value in MHWCumInt_xmap_BMaxChla_AL[i, :]:
        data.append([year, value, 'MHWCumInt_xmap_BMaxChla_AL'])

df = pd.DataFrame(data, columns=['Year', 'Value', 'Dataset'])

df['Dataset'] = df['Dataset'].replace({
    'BMaxChla_xmap_MHWCumInt_AL': 'BMaxChl-a xmap MHW Cum Intensity',
    'MHWCumInt_xmap_BMaxChla_AL': 'MHW Cum Intensity xmap BMaxChl-a'
})
...
sns.violinplot(x='Year_str', y='Value', hue='Dataset', data=df_mhwcumint_xmap, width=20, 
               palette={'MHW Cum Intensity xmap BMaxChl-a': 'lightpink'}, alpha=0.7, ax=ax, dodge=False, split=False)

mean_values = [np.nanmean(SCR_xmap_MHWCumInt_AL[i, :]) for i in range(SCR_xmap_MHWCumInt_AL.shape[0])]
...
plt.title('Geographical CCM: SoG and Alboran Sea (AL)', fontsize=16)
...
outfile = r'E:\...\Figures\Fig_S4\Fig_S4d.png'
fig.savefig(outfile, dpi=1200, bbox_inches='tight')



# Supplementary Fig. S4f (BMaxChl-a/SCR xmap Windiness)
libsizes_AL = np.load(r'E:\...\GCCM_outputs\libsizes/libsizes_AL.npy')
BMaxChla_xmap_Windiness_AL = np.load(r'E:\...\GCCM_outputs\BMaxChla_xmap_Windiness/BMaxChla_xmap_Windiness_AL.npy')
Windiness_xmap_BMaxChla_AL = np.load(r'E:\...\GCCM_outputs\BMaxChla_xmap_Windiness/Windiness_xmap_BMaxChla_AL.npy')
SCR_xmap_Windiness_AL = np.load(r'E:\...\GCCM_outputs\SCR_xmap_Windiness/SCR_xmap_Windiness_AL.npy')
...
for i, year in enumerate(years):
    for value in BMaxChla_xmap_Windiness_AL[i, :]:
        data.append([year, value, 'BMaxChla_xmap_Windiness_AL'])
    for value in Windiness_xmap_BMaxChla_AL[i, :]:
        data.append([year, value, 'Windiness_xmap_BMaxChla_AL'])

df = pd.DataFrame(data, columns=['Year', 'Value', 'Dataset'])

df['Dataset'] = df['Dataset'].replace({
    'BMaxChla_xmap_Windiness_AL': 'BMaxChl-a xmap Windiness',
    'Windiness_xmap_BMaxChla_AL': 'Windiness xmap BMaxChl-a'
})
...
sns.violinplot(x='Year_str', y='Value', hue='Dataset', data=df_bmaxchla_xmap, width=10, 
               palette={'BMaxChl-a xmap Windiness': 'lightblue'}, alpha=0.7, ax=ax, dodge=False, split=False)

sns.violinplot(x='Year_str', y='Value', hue='Dataset', data=df_Windiness_xmap, width=2, 
               palette={'Windiness xmap BMaxChl-a': 'lightpink'}, alpha=0.7, ax=ax, dodge=False, split=False)

mean_values = [np.nanmean(SCR_xmap_Windiness_AL[i, :]) for i in range(SCR_xmap_Windiness_AL.shape[0])]
...
outfile = r'E:\...\Figures\Fig_S4\Fig_S4f.png'
fig.savefig(outfile, dpi=1200, bbox_inches='tight')



## Supplementary Fig. S4e, g - Causal Inference through CCM (Sugihara et al., 2012) ##
...
ds_Windiness = xr.open_dataset(r'E:\...\Monthly_Data\Monthly_Windiness/Monthly_Windiness_AL.nc')
...
ds_Max_CHL = xr.open_dataset(r'E:\...\Monthly_Data\Monthly_MaxCHL/Monthly_MaxCHL_AL.nc')
...
## Supplementary Fig. S4e - Monthly BMaxChl-a xmap MHW Cum Intensity
...
tri_surf = ax.plot_trisurf(E_results, tau_results, rho_results, cmap=plt.cm.RdYlGn, alpha=0.8, vmin=0, vmax=0.6)
...
outfile = r'E:\...\Figures\Fig_S4\Fig_S4e.png'
fig.savefig(outfile, dpi=1200, bbox_inches='tight')



## Supplementary Fig. S4g - Monthly BMaxChl-a xmap Windiness
plt.rcParams.update({'font.size': 22})
...
outfile = r'E:\...\Figures\Fig_S4\Fig_S4g.png'
fig.savefig(outfile, dpi=1200, bbox_inches='tight')
