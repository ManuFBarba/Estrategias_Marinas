...
## Fig. 3c (Boxplots Bloom Max Chl-a)
...
for i, data in enumerate(data_demarcations):
    ...
    for j in range(len(years)):
        ...
        data_per_year.append(data_flat)
    bplot = axs[i].boxplot(
        data_per_year,
        positions=years,
        widths=0.6,
    ...
    whisker_vals = [whisker.get_ydata()[1] for whisker in bplot['whiskers']]
    ymin = max(min(whisker_vals), 1e-3)
    ymax = max(whisker_vals)
    axs[i].set_yscale('log')
    axs[i].set_ylim(ymin, ymax * 1.1)
    ...
    if i == 0:  
        axs[i].set_yticks([0.25, 1, 5])
        axs[i].set_yticklabels([0.25, '', 5], fontsize=45)
        axs[i].yaxis.set_major_formatter(ticker.FuncFormatter(lambda y, _: f'{int(y)}' if y == 5 else ('0.25' if y == 0.25 else '')))
        # axs[i].yaxis.set_major_formatter(ticker.FuncFormatter(lambda y, _: f'{y:.2f}' if y in [0.25, 5] else ''))
    if i == 1:  
        axs[i].set_yticks([0.32, 1, 5])
        axs[i].set_yticklabels([0.30, '', 5], fontsize=45)
        axs[i].yaxis.set_major_formatter(ticker.FuncFormatter(lambda y, _: f'{int(y)}' if y == 5 else ('0.32' if y == 0.32 else '')))
        # axs[i].yaxis.set_major_formatter(ticker.FuncFormatter(lambda y, _: f'{y:.2f}' if y in [0.32, 5] else ''))
    if i == 2:
        axs[i].set_yticks([0.15, 0.25, 0.75])
        axs[i].set_yticklabels([0.15, '', 0.75], fontsize=45)
        axs[i].set_ylabel('BMaxChl-a [mg·m$^{-3}$]', fontsize=45, labelpad=15)
        axs[i].yaxis.set_major_formatter(ticker.FuncFormatter(lambda y, _: f'{y:.2f}' if y in [0.15, 0.75] else ''))
    if i == 3:
        axs[i].set_yticks([0.25, 1, 5, 10])
        axs[i].set_yticklabels([0.25, '', '', 10], fontsize=45)
        axs[i].yaxis.set_major_formatter(ticker.FuncFormatter(lambda y, _: f'{int(y)}' if y == 10 else ('0.25' if y == 0.25 else '')))
        # axs[i].yaxis.set_major_formatter(ticker.FuncFormatter(lambda y, _: f'{y:.2f}' if y in [0.25, 10] else ''))
    if i == 4:
        axs[i].set_yticks([0.3, 1, 2])
        axs[i].set_yticklabels([0.3, '', 2], fontsize=45)
        axs[i].yaxis.set_major_formatter(ticker.FuncFormatter(lambda y, _: f'{int(y)}' if y == 2 else ('0.30' if y == 0.30 else '')))
        # axs[i].yaxis.set_major_formatter(ticker.FuncFormatter(lambda y, _: f'{y:.2f}' if y in [0.3, 2] else ''))
axs[-1].set_xticks(np.arange(1998, 2024, 4))
axs[-1].set_xticklabels(np.arange(1998, 2024, 4), fontsize=45)
...
## Fig. 3f (Boxplots Bloom Initiation)
years = np.arange(1998, 2024)
...
for i, data in enumerate(data_demarcations):
    ...
    axs[i].set_ylim(0, 365)
    axs[i].set_title(labels[i], fontsize=45)
    axs[i].set_ylabel('')
    axs[i].tick_params(axis='both', which='major', labelsize=45)
    
    axs[i].set_yticks([0, 30.5, 59.0, 91.5, 122.0, 152.5, 183.0, 213.5, 244.0, 274.5, 305.0, 336])
    axs[i].set_yticklabels(['Jan', '', '', '', '', '', '', '', '', '', '', 'Dec'], fontsize=45) 

    if i == 0:
        axs[i].set_ylim(0, 130)
        axs[i].set_yticks([0.1, 30.5, 59.0, 91.5, 122.0])
        axs[i].set_yticklabels(['Jan', '', '', '', 'May'], fontsize=45) 

    if i == 2:
        axs[i].set_ylabel(r'BInit $\mathrm{[days]}$', fontsize=45, labelpad=15)

    if i == 4:
        axs[i].set_yticks([0, 30.5, 59.0, 91.5, 122.0, 152.5, 183.0, 213.5, 244.0, 274.5, 305.0, 336])
        axs[i].set_yticklabels(['', '', 'Mar', '', '', '', '', '', '', '', '', 'Dec'], fontsize=45) 
...
## Fig. 3i (Boxplots Bloom Termination)
years = np.arange(1998, 2024)
...
for i, data in enumerate(data_demarcations):
    ...
    if i == 0:
        axs[i].set_ylim(62, 290)
        axs[i].set_yticks([62, 91.5, 122.0, 152.5, 183.0, 213.5, 244.0, 274.5])
        axs[i].set_yticklabels(['Mar', '', '', '', '', '', '', 'Oct'], fontsize=45) 

    if i == 1:
        axs[i].set_ylim(20, 336)
        axs[i].set_yticks([30.5, 59.0, 91.5, 122.0, 152.5, 183.0, 213.5, 244.0, 274.5, 305.0, 336])
        axs[i].set_yticklabels(['Feb', '', '', '', '', '', '', '', '', '', 'Dec'], fontsize=45) 

    if i == 2:
        axs[i].set_ylim(20, 336)
        axs[i].set_yticks([30.5, 59.0, 91.5, 122.0, 152.5, 183.0, 213.5, 244.0, 274.5, 305.0, 336])
        axs[i].set_yticklabels(['Feb', '', '', '', '', '', '', '', '', '', 'Dec'], fontsize=45) 
        axs[i].set_ylabel(r'BTerm $\mathrm{[days]}$', fontsize=45, labelpad=15)
...
## Fig. 3l (Boxplots Bloom Duration)
years = np.arange(1998, 2024)
...
for i, data in enumerate(data_demarcations):
    ...
    if i == 0:  
        axs[i].set_yticks([50, 150])
        axs[i].set_yticklabels([50, 150], fontsize=45) 
    if i == 1:  
        axs[i].set_yticks([50, 150])
        axs[i].set_yticklabels([50, 150], fontsize=45) 
    if i == 2:
        axs[i].set_yticks([50, 150])
        axs[i].set_yticklabels([50, 150], fontsize=45) 
        axs[i].set_ylabel(r'BDur [days]', fontsize=45, labelpad=15)
    if i == 3:
        axs[i].set_yticks([50, 150])
        axs[i].set_yticklabels([50, 150], fontsize=45) 
    if i == 4:
...
## Fig. 3o (Boxplots Bloom Cumulative Chl-a)
years = np.arange(1998, 2024)
...
for i, data in enumerate(data_demarcations):
    ...
    axs[i].tick_params(axis='both', which='major', labelsize=45)
    if i == 0:  
        axs[i].set_yticks([25, 50, 75, 125, 200])
        axs[i].set_yticklabels([25, '', '', '', 200], fontsize=45)
    if i == 1: 
        axs[i].set_ylim(20, ymax * 1.05)
        axs[i].set_yticks([25, 50, 75, 125, 200])
        axs[i].set_yticklabels([25, '', '', '', 200], fontsize=45)
    if i == 2:
        axs[i].set_ylim(10, ymax * 1.05)
        axs[i].set_yticks([10, 25, 50])
        axs[i].set_yticklabels([10, '', 50], fontsize=45)
        axs[i].set_ylabel(r'BCumChl-a $\mathrm{[mg \cdot m^{-3} \cdot days]}$', fontsize=45, labelpad=30)
        axs[i].yaxis.set_minor_formatter(ticker.NullFormatter())
        axs[i].yaxis.set_major_formatter(ticker.FuncFormatter(lambda y, _: f'{int(y)}' if y in [10,50] else ''))

    if i == 3:
        axs[i].set_yticks([25, 50, 75, 125, 200])
        axs[i].set_yticklabels([25, '', '', '', 200], fontsize=45)
    if i == 4:
        axs[i].set_ylim(15, ymax * 1.05)
        axs[i].set_yticks([25, 50, 75, 125])
        axs[i].set_yticklabels([25, '', '', 125], fontsize=45)
        axs[i].tick_params(axis='y', which='minor', left=False)
        axs[i].yaxis.set_minor_formatter(ticker.NullFormatter())
        axs[i].yaxis.set_major_formatter(ticker.FuncFormatter(lambda y, _: f'{int(y)}' if y in [25,125] else ''))
...
for i, data in enumerate(data_demarcations):
    ...
    if i == 0:  
        ...
        axs[i].set_yticklabels([0, '', '', '', '', 1], fontsize=45)
    if i == 1: 
        axs[i].set_ylim(0, 0.8)
        axs[i].set_ylim(0, 0.8)
        axs[i].set_yticks([0, 0.2, 0.4, 0.6, 0.8])
        axs[i].set_yticklabels([0, '', '', '', 0.8], fontsize=45)
    if i == 2:
        axs[i].set_ylim(0.2, 1)
        axs[i].set_yticks([0.2, 0.4, 0.6, 0.8, 1])
        axs[i].set_yticklabels([0.2, '', '', '', 1], fontsize=45)
        axs[i].set_ylabel(r'SCR $\mathrm{[r]}$', fontsize=45, labelpad=15)
    if i == 3:
        axs[i].set_ylim(0, 0.9)
        axs[i].set_yticks([0, 0.2, 0.4, 0.6, 0.8])
        axs[i].set_yticklabels([0, '', '', '', 0.8], fontsize=45)
    if i == 4:
        axs[i].set_ylim(0.2, 0.95)
        axs[i].set_yticks([0.2, 0.4, 0.6, 0.8])
        axs[i].set_yticklabels(['', 0.4, '', 0.8], fontsize=45)
